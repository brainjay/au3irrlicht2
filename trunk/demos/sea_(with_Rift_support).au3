...
_IrrStart($IRR_EDT_OPENGL, @DesktopWidth, @DesktopHeight, 32, true, _
...
;	$Static = AddStaticText( "This is a demonstration.", 10, 10, 200, 20, 0 )
	...
	global $Camera2 = _IrrAddFPSCamera(0, 50, 0.02, -1, __CreatePtrKeyMapArray($keyStruct), 4, 1, 0)

	_IrrSetNodePosition( $Camera2, 0, 40, 520 ) ;setting position
...
_IrrSetNodeTriangleSelector($Camera1, $WaterSelector)
	...
	dim $Cam2Animator = _IrrAddCollisionAnimator( $ShipSelector, $Camera2, 5, 8, 5, 0, $Gravity, 0, 0, 0, 0 ) ;character can collide it
...
local $bOverWater = true;
	...
	_IrrHideMouse()
	While _IrrRunning() and $gameLoop
...
;~ ;				SetGravity( $MapCam1Animator, 0, $Gravity, 0 )
		...
		ElseIf $y < 0 and $bOverWater = true then
				$bOverWater = False
 				;_IrrRemoveAnimator($Camera1, $MapCam1Animator)
 				;$MapCam1Animator = _IrrAddCollisionAnimator( $MapSelector, $Camera1, 6, 10, 6, 0.0, 0.0, 0.0, 0, 5, 0 )
