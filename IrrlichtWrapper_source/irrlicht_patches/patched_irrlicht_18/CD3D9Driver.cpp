...
void CD3D9Driver::createMaterialRenderers()
{
	...
	// IrrlichtWrapper
	// add four greyscale detail mapping
	addAndDropMaterialRenderer(new CD3D9MaterialRenderer_FOUR_DETAIL_MAP(pID3DDevice, this));

	// add an alpha ref transparency effect that adds to the vertex color so it can be tinted
	addAndDropMaterialRenderer(new CD3D9MaterialRenderer_TRANSPARENT_ADD_ALPHA_CHANNEL_REF(pID3DDevice, this));

	// add an alpha ref transparency effect that adds to the vertex color so it can be tinted
	addAndDropMaterialRenderer(new CD3D9MaterialRenderer_TRANSPARENT_ADD_ALPHA_CHANNEL(pID3DDevice, this));
	// IrrlichtWrapper End
}
...
{
	//! IrrlichtWrapper
	printf( "Begining createScreenShot\n" );
	//! IrrlichtWrapper End
	...
	// IrrlichtWrapper
	printf( "Creating surface for front buffer (%d x %d) @ %p\n",
			displayMode.Width,
			displayMode.Height,
			&lpSurface);
	//! IrrlichtWrapper End
	...
	// IrrlichtWrapper
	{
		printf( "failed recovering surface\n" );
	// IrrlichtWrapper End
		return 0;
	// IrrlichtWrapper
	}
	printf( "recovered surface (%d)\n", hr );
	// IrrlichtWrapper End

	// read the front buffer into the image surface
	// IrrlichtWrapper
	printf( "reading front buffer to surface\n" );
	// IrrlichtWrapper End
	if (FAILED(hr = pID3DDevice->GetFrontBufferData(0, lpSurface)))
	{
		// IrrlichtWrapper
		printf( "failed read. releasing surface\n" );
		// IrrlichtWrapper End
		lpSurface->Release();
		// IrrlichtWrapper
	...
	RECT clientRect;
	{
		...
		// IrrlichtWrapper
		printf( "Client to Screen handle=%d\n", getExposedVideoData().D3D9.HWnd );
		// IrrlichtWrapper End
		ClientToScreen((HWND)getExposedVideoData().D3D9.HWnd, &clientPoint);
		// IrrlichtWrapper
		printf( "recovered client point\n" );
		// IrrlichtWrapper End
	...
	// IrrlichtWrapper
	printf( "locking surface rectangle\n" );
	// IrrlichtWrapper End
	if (FAILED(lpSurface->LockRect(&lockedRect, &clientRect, D3DLOCK_READONLY)))
	{
		// IrrlichtWrapper
		printf( "failed locking surface rectangle. releasing surface\n" );
		// IrrlichtWrapper End
		lpSurface->Release();
		// IrrlichtWrapper
		printf( "released surface\n" );
		// IrrlichtWrapper End
		return 0;
	}
	...
	// IrrlichtWrapper
	printf( "Creating a new image (%u x %u)\n", ScreenSize.Width, ScreenSize.Height );
	// IrrlichtWrapper End
	...
	if (newImage)
	{
		...
		// IrrlichtWrapper
		printf( "locking the new image\n" );
		// IrrlichtWrapper End
		u32* dP = (u32*)newImage->lock();
		u8 * sP = (u8 *)lockedRect.pBits;
	// IrrlichtWrapper
	printf( "new image locked at %p, bits at %p\n", dP, sP );
	// IrrlichtWrapper End
	...
	// IrrlichtWrapper
	printf( "checking display mode format %d\n", displayMode.Format );
	// IrrlichtWrapper End
	if (D3DFMT_X8R8G8B8 == displayMode.Format && (0xFF000000 != (*dP & 0xFF000000)))
		{
			// IrrlichtWrapper
			printf( "copying pixels for D3DFMT_X8R8G8B8\n" );
			// IrrlichtWrapper End
		...
		{
			// IrrlichtWrapper
			printf( "copying pixels for NOT D3DFMT_X8R8G8B8\n" );
			// IrrlichtWrapper End
			for (u32 y = 0; y < shotSize.Height; ++y)
			{
	...
	// IrrlichtWrapper
	printf( "unlocking surface rectangle\n" );
	// IrrlichtWrapper End
	lpSurface->UnlockRect();

	// release the image surface
	// IrrlichtWrapper
	printf( "releasing surface rectangle\n" );
	// IrrlichtWrapper End
	lpSurface->Release();

	// return status of save operation to caller
	// IrrlichtWrapper
	printf( "returning image at %p\n", newImage );
	// IrrlichtWrapper End
	return newImage;
}
